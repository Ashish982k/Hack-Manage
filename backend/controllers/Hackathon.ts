...
const STAGE_TYPES: StageType[] = ["SUBMISSION", "EVALUATION", "FINAL"];
const SCHEDULE_TYPES: ScheduleType[] = ["entry", "breakfast", "lunch", "dinner"];
...
const normalizeEmailList = (input: unknown[]) =>
  Array.from(
    new Set(
      input
...
const parseStageType = (value: unknown): StageType | null => {
  if (typeof value !== "string") {
    return null;
  }

  const normalized = value.trim().toUpperCase() as StageType;
...
const parseScheduleType = (value: unknown): ScheduleType | null => {
  ...
  const normalized = value.trim().toLowerCase() as ScheduleType;
...
export const newHackathon = async (c: AppContext) => {
  try {
    ...
    const stagesData = JSON.parse((body.stages as string) || "[]") as StageInput[];
    ...
    const problems = JSON.parse((body.problemStatements as string) || "[]");
...
export const saveHackathonSchedules = async (c: AppContext) => {
  try {
    ...
    const schedules = Array.isArray((body as { schedules?: unknown })?.schedules)
    ...
    const seenTypes = new Set<ScheduleType>();
    ...
    for (const item of schedules) {
      ...
      if (!startTime || !isDateTimeValue(startTime)) {
        return c.json(
          { message: "Each schedule must have a valid start time" },
          400,
        );
      }
      ...
      if (!endTime || !isDateTimeValue(endTime)) {
        return c.json(
          { message: "Each schedule must have a valid end time" },
          400,
        );
      }
    ...
    for (const scheduleType of SCHEDULE_TYPES) {
      if (seenTypes.has(scheduleType)) continue;
      await db
        .delete(hackathonSchedules)
        .where(
          and(
            eq(hackathonSchedules.hackathonId, hackathonId),
            eq(hackathonSchedules.type, scheduleType),
          ),
        );
    }
  ...
  try {
    ...
    const participant = await db.query.hackathonParticipants.findFirst({
      where: and(
        ...
        eq(hackathonParticipants.userId, currentUser.id),
      ),
    });
