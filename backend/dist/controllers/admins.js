...
import { evaluations, hackathonParticipants, hackathonRoles, hackathons, problemStatements, shortlistedTeams, stages, submissions, teamMembers, teams, user, qrCodes, } from "../src/db/schema.js";
    ...
    try {
        ...
        if (!isCreator) {
            const role = await db.query.hackathonRoles.findFirst({
                where: and(eq(hackathonRoles.hackathonId, id), eq(hackathonRoles.userId, currentUser.id), eq(hackathonRoles.role, "admin")),
            });
        ...
        const teamIds = (await db
            .select({ id: teams.id })
            .from(teams)
            .where(eq(teams.hackathonId, id))).map((t) => t.id);
        if (teamIds.length) {
            const submissionIds = (await db
                .select({ id: submissions.id })
                .from(submissions)
                .where(inArray(submissions.teamId, teamIds))).map((s) => s.id);
