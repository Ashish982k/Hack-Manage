...
export const sendWinnerEmails = async (hackathonId) => {
    try {
        ...
        const shortlisted = await db
            ...
            teamName: teams.name,
            totalScore: sql `COALESCE(SUM(${evaluations.total}), 0)`,
        })
            .from(submissions)
            .innerJoin(teams, eq(submissions.teamId, teams.id))
            .innerJoin(evaluations, eq(submissions.id, evaluations.submissionId))
            .innerJoin(shortlistedTeams, and(eq(shortlistedTeams.teamId, submissions.teamId), eq(shortlistedTeams.stageId, finalStage.id)))
            .where(and(eq(shortlistedTeams.hackathonId, hackathonId), eq(submissions.stageId, finalStage.id)))
