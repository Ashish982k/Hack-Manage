...
export const generateQR = async (c) => {
    ...
    const team = await db
        ...
        .where(and(eq(teamMembers.userId, userId), eq(teams.hackathonId, hackathonId)))
        .limit(1);
    if (team.length === 0) {
    ...
    const finalRoundTeam = await db.query.shortlistedTeams.findFirst({
        where: and(eq(shortlistedTeams.teamId, teamId), eq(shortlistedTeams.hackathonId, hackathonId), eq(shortlistedTeams.stageId, finalStageId)),
    });
    if (!finalRoundTeam) {
    ...
    for (const schedule of schedules) {
        const existing = await db.query.qrCodes.findFirst({
            where: and(eq(qrCodes.hackathonId, hackathonId), eq(qrCodes.userId, userId), eq(qrCodes.teamId, teamId), eq(qrCodes.type, schedule.type)),
        });
        if (existing) {
            qrData.push({
                type: schedule.type,
