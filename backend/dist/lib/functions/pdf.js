...
export const generateCertificatePdf = async ({ winnerName, teamName, eventName, eventYear, position, date, location, }) => {
    ...
    try {
        const cursiveFontPath = path.resolve(process.cwd(), "..", "DancingScript.ttf");
    ...
    const drawCenter = (text, y, size, font, color = rgb(0.12, 0.12, 0.12)) => {
        const textWidth = font.widthOfTextAtSize(text, size);
        const x = (width - textWidth) / 2;
        page.drawText(text, { x, y, size, font, color });
    };
    const drawMixedCenter = (segments, y, size) => {
        const totalWidth = segments.reduce((sum, seg) => sum + seg.font.widthOfTextAtSize(seg.text, size), 0);
    ...
    const drawLeft = (text, x, y, size, font, color = rgb(0.12, 0.12, 0.12)) => {
        page.drawText(text, { x, y, size, font, color });
    };
    // ── Outer border ──────────────────────────────────────────────────────────
    page.drawRectangle({
    ...
    drawCenter("Certificate of Participation", height - 148, 18, fontRegular, rgb(0.35, 0.35, 0.35));
    // ── Decorative divider ────────────────────────────────────────────────────
    ...
    const positionSuffix = position === 1
        ? "1st"
        : position === 2
            ? "2nd"
            : position === 3
    ...
    const line1Width = line1Segments.reduce((sum, seg) => sum + seg.font.widthOfTextAtSize(seg.text, 13), 0);
    ...
    const margin = 60;
    const sigSpacing = (width - 2 * margin - signatories.length * sigLineLen) /
