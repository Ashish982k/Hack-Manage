...
export const resolveSubmissionStageId = async (hackathonId, stage) => {
    ...
    const [linkedSubmissionStage] = stage.startTime
        ? await db
            .select({ id: stages.id })
            .from(stages)
            .where(and(eq(stages.hackathonId, hackathonId), eq(stages.type, "SUBMISSION"), sql `datetime(${stages.startTime}) <= datetime(${stage.startTime})`))
            .orderBy(desc(sql `datetime(${stages.startTime})`))
            .limit(1)
        : [];
    if (linkedSubmissionStage)
        return linkedSubmissionStage.id;
