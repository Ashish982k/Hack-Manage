...
import { PDFDocument, StandardFonts, rgb, PDFName, type PDFPage } from "pdf-lib";
...
export const generateTeamLogsReportPdf = async ({
  title,
  eventName,
  eventDateLabel,
  adminEmail,
  teams,
}: TeamReportPdfInput): Promise<Buffer> => {
...
export const generateFinalReportPdf = async ({
  ...
  foodAvailableCount,
  foodUsedCount,
}: FinalReportPdfInput): Promise<Buffer> => {
  const pdfDoc = await PDFDocument.create();
...
const addLink = (page: PDFPage, url: string, rect: { x: number, y: number, width: number, height: number }) => {
  const doc = page.doc;
  ...
  let annots = page.node.get(PDFName.of('Annots')) as any;
  if (!annots) {
...
export type TeamAnalyticsPdfInput = {
  title: string;
  eventName: string;
  eventDateLabel: string;
  adminEmail: string;
...
export const generateTeamAnalyticsPdf = async ({
  title,
  eventName,
  eventDateLabel,
  adminEmail,
  teams,
}: TeamAnalyticsPdfInput): Promise<Buffer> => {
  const pdfDoc = await PDFDocument.create();
  const fontRegular = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const fontBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const generatedAt = new Date().toLocaleString();
...
export type JudgeAnalyticsRow = {
  teamName: string;
  evaluations: JudgeEvaluation[];
};

export type JudgeAnalyticsPdfInput = {
  title: string;
  eventName: string;
  eventDateLabel: string;
  adminEmail: string;
  teams: JudgeAnalyticsRow[];
};

export const generateJudgeAnalyticsPdf = async ({
  title,
  eventName,
  eventDateLabel,
  adminEmail,
  teams,
}: JudgeAnalyticsPdfInput): Promise<Buffer> => {
  const pdfDoc = await PDFDocument.create();
  const fontRegular = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const fontBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const generatedAt = new Date().toLocaleString();
